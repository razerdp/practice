...
public class Log {
    ...
    public static synchronized void setLogOutFile(File logOutFile) throws IOException {
        ...
        if (Log.logOutFile != null) {
            try {
                logOutFileStream = new RandomAccessFile(Log.logOutFile, "rw");
                logOutFileStream.seek(Log.logOutFile.length());
            } catch (IOException e) {
                closeStream(logOutFileStream);
                logOutFileStream = null;
                throw e;
